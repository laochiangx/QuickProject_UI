 ...
 this.$nextTick(() => {
      ...
      this.nowDate = `${month}-${day} ${hour}:${minute}:${second}`;
        ...
        systemInfoApi.getOverView().then(res => {
          if(res.data)
          {//res.data.metaSpace
              this.datas.push(parseFloat(res.data.metaSpace.committed/1024/1024).toFixed(2)) ;
              this.datass.push(parseFloat(res.data.metaSpace.used/1024/1024).toFixed(2));
              this.data3.push(parseFloat(res.data.metaSpace.init/1024/1024).toFixed(2));
          }else
          {
    ...
    initChart() {
      ...
      this.chart.setOption({
        ...
        legend: {
          ...
          itemHeight: 5,
          itemGap: 13,
          data: ['committed（M）', 'used（M）','init（M）'],
          right: '4%',
          textStyle: {
            fontSize: 12,
            color: '#000000'
