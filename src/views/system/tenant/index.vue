<template>
  <el-card shadow="never">
    ...
    <div>
      <el-table ref="tableCot" v-loading="loading" element-loading-text="请稍后..." :data="tenantData" border
                style="width: 100%" :max-height="customTableHeight">
        <el-table-column type="index" width="40"/>
        <el-table-column prop="tenantId" label="租户编号" min-width="120" show-overflow-tooltip/>
        ...
        <el-table-column label="状态" min-width="100">
          <template slot-scope="scope">
            <el-tooltip :content="scope.row.delFlag='0'?'已启用':'已禁用'" placement="top">
              <el-switch
        ...
        <el-table-column fixed="right" label="操作" width="130">
          <template slot-scope="scope">
            ...
            <el-tag  style="cursor:pointer;" size="mini" type="danger"
...
import addEdit from '@/views/system/tenant/addEdit'
  ...
  methods: {
    switchChange(row) {
      ...
      this.$confirm(text, '提示', {
        ...
        systemTenantApi.status(row.id,status).then(res => {
    ...
    getList() {
      this.loading = true
      systemTenantApi.list(this.listQuery).then(res => {
    ...
    remove(row) {
      this.$confirm('此操作将永久删除该租户, 是否继续?', '提示', {
        ...
        systemTenantApi.remove(row.id).then(res => {
          this.$message({
            type: 'success',
            message: '删除成功'
          })
